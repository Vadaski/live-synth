...
import { applyEffects, applyTrackEffects, disposeAllTrackEffects } from "./effects.js";
...
type Schedulable = { dispose: () => void };
type ScaleState = { root: string; mode: string } | null;
type VoiceCommand = Extract<Command, { type: VoiceType }>;
type ArpCommand = Extract<Command, { type: "arp" }>;
type DrumCommand = Extract<Command, { type: "drum" }>;
type VolCommand = Extract<Command, { type: "vol" }>;
type OctCommand = Extract<Command, { type: "oct" }>;
type SwingCommand = Extract<Command, { type: "swing" }>;
type EnvCommand = Extract<Command, { type: "env" }>;
type FilterCommand = Extract<Command, { type: "filter" }>;

const DEFAULT_DB: Record<VoiceRole, number> = { synth: -10, bass: -8, pad: -12, lead: -8, arp: -9 };
const NOTE_RE = /^([A-G][#b]?)(-?\d+)$/;
const ARP_MODE: Record<ArpCommand["mode"], "up" | "down" | "upDown" | "random"> = {
  up: "up",
  down: "down",
  updown: "upDown",
  random: "random",
...
const DRUM_NAMES = new Set<DrumName>([
  "kick",
  "snare",
  "hat",
  "clap",
...
export function setOnBeat(cb: (beat: number) => void): void {
  onBeatCallback = cb;
}
export function setOnNote(cb: (note: string) => void): void {
  onNoteCallback = cb;
}

function clearScheduled(): void {
  for (const item of scheduled) item.dispose();
  scheduled = [];
}

function shiftOctave(note: string, octaves: number): string {
  const m = NOTE_RE.exec(note);
  return !octaves || !m ? note : `${m[1]}${Number(m[2]) + octaves}`;
}
...
function triggerVoice(
  ...
  const synth = entry.synth as unknown as {
    ...
    triggerAttack?: (note: string, t: number | undefined, vel?: number) => void;
  };
  if (typeof synth.triggerAttackRelease === "function")
    synth.triggerAttackRelease(noteArg, durSeconds, time, velocity);
  else if (typeof synth.triggerAttack === "function" && typeof noteArg === "string")
    synth.triggerAttack(noteArg, time, velocity);
}
  ...
  const seq = new Tone.Sequence(
    (time, value) => {
      ...
      triggerVoice(
        ...
        "8n",
        velocity,
        noteDurationSteps(value.index, pattern),
        time,
      );
...
function ensureBeatTracker(): void {
  const transport = Tone.getTransport() as ReturnType<typeof Tone.getTransport> & {
    _beatTracker?: Tone.Loop;
  };
...
export function applyCommands(commands: Command[]): void {
  ...
  const volumes = new Map<string, number>(
    commands.filter((c): c is VolCommand => c.type === "vol").map((c) => [c.voice, c.value]),
  );
  ...
  for (const cmd of commands) {
    if (cmd.type === "env") {
      const c = cmd as EnvCommand;
      setVoiceEnvelope(c.target, c.params);
    } else if (cmd.type === "filter") {
      const c = cmd as FilterCommand;
      setVoiceFilter(c.target, c.filterType, c.frequency, c.Q);
    }
  }

  disposeAllTrackEffects();
  for (const id of voiceIds) applyTrackEffects(id, commands);
